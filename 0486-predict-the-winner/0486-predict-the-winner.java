...
class Solution {
    ...
    public boolean solve(int i, int j, int[] arr, int p1s, int p2s, boolean p1Turn) {
        ...
        if (p1Turn) {
            ...
            return solve(i + 1, j, arr, p1s + arr[i], p2s, false) || solve(i, j - 1, arr, p1s + arr[j], p2s, false);
        } else {
            // Player 2 also plays optimally.
            return solve(i + 1, j, arr, p1s, p2s + arr[i], true) && solve(i, j - 1, arr, p1s, p2s + arr[j], true);
        }
    }
}